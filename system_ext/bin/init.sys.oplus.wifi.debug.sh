...
function collectwifidmesgrooot(){
        ...
        if [[ $platform == *"mt"* ]] || [[ $platform == *"Mt"*  ]] || [[ $platform == *"MT"*  ]];then
            ...
            if [ "x${platform}" == "xlito" ];then
                echo "lito got from pdmapper"
                reason=`dmesg | grep PD_ERR | sed -n '$p'`
                reason=`echo ${reason#*PD_ERR] }`
            elif [ "x${platform}" == "xholi" ] || [ "x${platform}" == "xblair" ];then
                echo "holi/blair got from pdmapper"
                reason=`dmesg | grep PD_ERR | sed -n 's/^.*EF:\(.*\)/\1/p'`
                reason=`echo ${reason#*PD_ERR] }`
            else
...
function wifi_trace_log_catch() {
    if [ "x$(pidof system_server)" != "x" ];then
        /system/bin/debuggerd -j $(pidof system_server) > /data/vendor/wifi/wifi_trace/system_server.log
    fi
    if [ "x$(pidof android.hardware.wifi-service)" != "x" ];then
        /system/bin/debuggerd -b $(pidof android.hardware.wifi-service) > /data/vendor/wifi/wifi_trace/android_hardware_wifi-service.log
    fi
    if [ "x$(pidof wificond)" != "x" ];then
        /system/bin/debuggerd -b $(pidof wificond) > /data/vendor/wifi/wifi_trace/wificond.log
    fi
    if [ "x$(pidof wpa_supplicant)" != "x" ];then
        /system/bin/debuggerd -b $(pidof wpa_supplicant) > /data/vendor/wifi/wifi_trace/wpa_supplicant.log
    fi
    if [ "x$(pidof vendor.oplus.hardware.wifi-aidl-service)" != "x" ];then
        /system/bin/debuggerd -b $(pidof vendor.oplus.hardware.wifi-aidl-service) > /data/vendor/wifi/wifi_trace/vendor_oplus_hardware_wifi-aidl-service.log
    fi
    /system/bin/ps -A > ps.log
    chmod -R 777 /data/vendor/wifi/wifi_trace
    chown system:system -R /data/vendor/wifi/wifi_trace
}

function wifi_cold_reset() {
    echo 2 > /sys/module/cnss2/parameters/pci_reset
}
