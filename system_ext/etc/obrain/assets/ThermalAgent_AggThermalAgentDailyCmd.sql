...
from (
	select json_group_array(json(oneline)) as histogram
	from (
		...
		select json_group_array(json(oneline)) as top_app
		from (
					...
					from PARAM_VAR, agg_thermalAgent_hourly T1
						inner join (
							...
							from (
								...
								having SumVector2II('sum(zone$0)*$1', ThermalList, OneList) >= 60000
							)
							where idx <= 10
						) T2
